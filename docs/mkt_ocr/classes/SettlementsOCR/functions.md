...
- **Llamada a la API:** Envía el archivo y su nombre al servicio externo a través de la función utilitaria [ocr_consultation](../../utils/api_ocr/functions.md#ocr-consultation).
...
- **Mapeo de Datos:** Si el proceso es exitoso, actualiza el campo `used_document_ocr = True` y recopila variables en el diccionario `vals`:
    - Formatea la `fecha` mediante la utilidad [date_format](../../utils/api_ocr/functions.md#date-format).
    - Asigna el RUC o DNI (`documento_cliente`) al campo `dni_ruc` o `partner`.
    - Formatea el `numero_factura` usando [formatInvoiceNumber](../../utils/api_ocr/functions.md#format-invoice-number) para estandarizar la entrada.
